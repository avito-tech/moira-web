...
class TriggerContainer extends React.Component<Props, State> {
    ...
    async setTriggerMaintenance(triggerId: string, maintenance: Maintenance): Promise<void> {
        this.setState({ loading: true });
        const maintenanceTime = getMaintenanceTime(maintenance);
        await this.props.moiraApi.setTriggerMaintenance(
            triggerId,
            maintenanceTime > 0
                ? moment
                      .utc()
    ...
    render(): React.Node {
        ...
        const { page } = self.parseLocationSearch(location.search);
                ...
                {trigger != null && triggerState != null && config != null && (
                    <LayoutPlate>
                        <TriggerInfo
                            ...
                            onMaintenanceChange={newMaintenance => {
                                self.setTriggerMaintenance(trigger.id, newMaintenance);
                            }}
                        />
                    </LayoutPlate>
                )}
